...
// Tests the in-memory rate limiter for API protection
...
describe('Rate Limiter', () => {
  ...
  describe('checkRateLimit', () => {
    ...
    describe('edge cases', () => {
      ...
      it('should handle unicode identifier', () => {
    ...
    describe('resetIn accuracy', () => {
      it('should return 0 or positive for resetIn when blocked', () => {
        const id = getUniqueId();
        const config: RateLimitConfig = { limit: 1, windowMs: 60000 };
  ...
  describe('RATE_LIMITS presets', () => {
    ...
    it('should work with checkRateLimit', () => {
      const id = getUniqueId();

      const result = checkRateLimit(id, RATE_LIMITS.standard);
  ...
  describe('Integration Scenarios', () => {
    it('should handle realistic API usage pattern', () => {
    ...
    it('should block abuse pattern', () => {
      const ip = getUniqueId();

      // Rapid-fire requests (abuse)
      for (let i = 0; i < 30; i++) {
        checkRateLimit(ip, RATE_LIMITS.standard);
      }
    ...
    it('should handle multiple endpoints with different limits', () => {
      ...
      for (let i = 0; i < 5; i++) {
        checkRateLimit(authKey, RATE_LIMITS.strict);
      }

      // Auth should be blocked
      expect(checkRateLimit(authKey, RATE_LIMITS.strict).success).toBe(false);

      // But standard endpoint should still work
      const apiKey = `api:${ip}`;
      expect(checkRateLimit(apiKey, RATE_LIMITS.standard).success).toBe(true);
    });
