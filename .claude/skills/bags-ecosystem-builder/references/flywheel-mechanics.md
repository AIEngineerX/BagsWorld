...
async function getFlywheelStats() {
  ...
  return {
    ...
    last24hFees: feeHistory.filter(f => f.timestamp > Date.now() - 86400000)
...
async function loadFlywheelStats() {
  ...
  document.getElementById('total-fees').textContent = `${stats.totalFeesCollected.toFixed(2)} SOL`;
  document.getElementById('bags-burned').textContent = `${stats.totalBagsBurned.toLocaleString()} BAGS`;
