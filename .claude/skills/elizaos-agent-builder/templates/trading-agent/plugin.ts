...
import { Plugin, Action, Provider, Evaluator, IAgentRuntime, Memory, State } from "@elizaos/core";
  ...
  get: async (runtime: IAgentRuntime, message: Memory, state?: State) => {
    ...
    try {
      ...
      const priceText = prices
        .map(
          (p) =>
            `${p.symbol}: $${p.price} (${p.change24h}% 24h, Vol: $${formatNumber(p.volume24h)})`
        )
        .join("\n");
...
const checkPriceAction: Action = {
  ...
  validate: async (runtime: IAgentRuntime, message: Memory, state: State) => {
    const text = message.content.text.toLowerCase();
    return (
      text.includes("price") || text.includes("how much") || text.match(/\$[a-zA-Z]+/) !== null
    );
  },
...
const analyzeAction: Action = {
  ...
  validate: async (runtime: IAgentRuntime, message: Memory, state: State) => {
    const text = message.content.text.toLowerCase();
    return text.includes("analyze") || text.includes("research") || text.includes("investigate");
  },
