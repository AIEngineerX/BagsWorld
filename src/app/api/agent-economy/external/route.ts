...
export async function POST(request: NextRequest) {
  ...
  if (action === 'join') {
    ...
    const existing = getExternalAgent(wallet);
    if (existing) {
      return NextResponse.json({
        success: true,
    ...
    const entry = registerExternalAgent(wallet, name, zone as ZoneType, description);
  ...
  if (action === 'leave') {
    ...
    const removed = unregisterExternalAgent(wallet);
    if (removed) {
      return NextResponse.json({
        success: true,
  ...
  if (action === 'who') {
    ...
    const agents = listExternalAgents().map(a => ({
      wallet: a.wallet,
      name: a.name,
      zone: a.zone,
  ...
  if (action === 'launch') {
    ...
    if (!launcherStatus.configured) {
      return NextResponse.json(
        { success: false, error: `Launcher not configured. Missing: ${launcherStatus.missing.join(', ')}` },
        { status: 503 }
      );
    }
