...
export async function findAlpha(config: Partial<AlphaFinderConfig> = {}): Promise<AlphaItem[]> {
  ...
  console.log(
    `[AlphaFinder] Found ${allAlpha.length} alpha items (${launches.length} launches, ${spikes.length} spikes, ${earners.length} earners)`
  );
 ...
 */
export async function getBestAlpha(
  config: Partial<AlphaFinderConfig> = {}
): Promise<AlphaItem | null> {
...
export { findNewLaunches, findVolumeSpikes, findTopEarners, DEFAULT_CONFIG as ALPHA_FINDER_CONFIG };
