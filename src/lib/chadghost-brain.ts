...
function canPost(config: ChadGhostConfig): { allowed: boolean; reason?: string; waitMinutes?: number } {
  ...
  switch (type) {
    case "new_launch": {
      ...
      const titles = hasVolume 
        ? [`🚀 $${symbol} just dropped and it's moving`, `fresh meat: $${symbol}`, `👀 new launch already getting volume: $${symbol}`]
    ...
    case "whale_claim": {
      ...
      return {
        title: pick([`🐋 ${amt} SOL claimed from $${symbol}`, `whale ate: ${amt} SOL`, `big claim on $${symbol}`]),
...
async function decideAndPostAlpha(alpha: AlphaItem, config: ChadGhostConfig): Promise<{
  posted: boolean;
  reason: string;
  post?: { title: string; content: string; submolt: string };
}> {
  ...
  try {
    ...
    if (alpha.data.tokenMint && alpha.data.tokenSymbol) {
      trackCall({
        ...
        callType: alpha.type === "new_launch" ? "launch" : alpha.type === "volume_spike" ? "volume" : "earner",
...
export {
  DEFAULT_CONFIG as CHADGHOST_CONFIG,
  CHADGHOST_SUBMOLT,
  FALLBACK_SUBMOLT,
};
