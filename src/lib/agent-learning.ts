...
export function analyzePerformance(): void {
  ...
  sortedTypes.forEach((t, i) => {
    learningState.adaptiveConfig.preferredPostTypes[t.type] = 1.0 + (0.2 * (sortedTypes.length - i) / sortedTypes.length);
  });
  ...
  sortedHours.forEach((h, i) => {
    learningState.adaptiveConfig.preferredHours[h.hour] = 1.0 + (0.3 * (sortedHours.length - i) / sortedHours.length);
  });
  ...
  sortedSubmolts.forEach((s, i) => {
    learningState.adaptiveConfig.preferredSubmolts[s.submolt] = 1.0 + (0.25 * (sortedSubmolts.length - i) / sortedSubmolts.length);
  });
