...
// ============================================================================
// UFOS & SPACESHIPS — Multiple flying craft variants
// ============================================================================
function generateUFOs(scene: Phaser.Scene): void {
  const s = SCALE;

  // === UFO VARIANT 0: Classic saucer (small, fast) ===
  {
    const g = scene.make.graphics({ x: 0, y: 0 });
    ...
    // Teal underglow
    g.fillStyle(ALIEN.teal, 0.2);
    g.fillEllipse(cx, cy + Math.round(4 * s), Math.round(22 * s), Math.round(6 * s));
  ...
  // === UFO VARIANT 1: Large mothership (slow, imposing) ===
  {
    const g = scene.make.graphics({ x: 0, y: 0 });
    ...
    // Massive teal glow underneath
    g.fillStyle(ALIEN.teal, 0.12);
    g.fillEllipse(cx, cy + Math.round(6 * s), Math.round(44 * s), Math.round(10 * s));
  ...
  // === UFO VARIANT 2: Triangular craft (black triangle / TR-3B style) ===
  {
    const g = scene.make.graphics({ x: 0, y: 0 });
    ...
    g.fillStyle(PALETTE.brightRed, 0.8);
    g.fillRect(cx - Math.round(1 * s), Math.round(2 * s), Math.round(3 * s), Math.round(3 * s));
    g.fillStyle(ALIEN.teal, 0.8);
    g.fillRect(Math.round(2 * s), h - Math.round(4 * s), Math.round(3 * s), Math.round(3 * s));
    g.fillRect(w - Math.round(5 * s), h - Math.round(4 * s), Math.round(3 * s), Math.round(3 * s));

    // Center orb
    g.fillStyle(ALIEN.teal, 0.3);
    g.fillRect(cx - Math.round(3 * s), Math.round(8 * s), Math.round(6 * s), Math.round(6 * s));
    g.fillStyle(ALIEN.teal, 0.6);
    g.fillRect(cx - Math.round(2 * s), Math.round(9 * s), Math.round(4 * s), Math.round(4 * s));

    g.generateTexture("disclosure_ufo_2", w, h);
    g.destroy();
  }

  // === UFO VARIANT 3: Tiny scout drone (fastest, smallest) ===
  {
    const g = scene.make.graphics({ x: 0, y: 0 });
    const w = Math.round(14 * s);
    const h = Math.round(8 * s);
    const cx = w / 2;
    const cy = h / 2;
  ...
  // === SPACESHIP: Alien fighter (angular, fast) ===
  {
    const g = scene.make.graphics({ x: 0, y: 0 });
