...
const STUU_TOPICS = [
  {
    ...
    icon: "🎧",
    content:
      "Got an issue? Let's figure it out together. Most problems have simple solutions - we just need to find them.",
  },
  {
    title: "Getting Started",
    icon: "🚀",
    content:
      "New to Bags.fm? Connect your wallet, browse trending tokens, or launch your own. The community is here to help.",
  },
  {
    title: "Transactions",
    icon: "💸",
    content:
      "Transaction pending? Usually it's network congestion or RPC issues. Check Solscan for status. Priority fees help during busy periods.",
  },
  {
    title: "Fee Claims",
    icon: "💰",
    content:
      "Fee claims are on-chain and instant. If your claim seems stuck, check the explorer. The contract handles all the math automatically.",
  },
  {
    title: "Community",
    icon: "👥",
    content:
      "Join the Discord, follow on Twitter, ask questions. The Bags community is one of the most helpful in crypto. We're all gonna make it.",
  },
];

export function StuuChat() {
  const [messages, setMessages] = useState<StuuMessage[]>([]);
  ...
  const addMessage = useCallback((message: StuuMessage) => {
    setMessages((prev) => [...prev.slice(-30), message]);
  }, []);

  const handleTopicClick = (topic: (typeof STUU_TOPICS)[0]) => {
    addMessage({
      id: `${Date.now()}-info`,
      type: "info",
  ...
  const sendToStuu = useCallback(
    async (userMessage: string) => {
      ...
      try {
        ...
        const messageText = data.response || "happy to help, what's up?";
        ...
        console.error("Stuu chat error:", error);
        addMessage({
          id: `${Date.now()}-stuu`,
          type: "stuu",
  ...
  useEffect(() => {
    const handleStuuClick = () => {
      setIsOpen(true);
      if (messages.length === 0) {
        sendToStuu("initializing...");
      }
    };

    window.addEventListener("bagsworld-stuu-click", handleStuuClick);
    return () => {
      window.removeEventListener("bagsworld-stuu-click", handleStuuClick);
    };
  }, [messages.length, sendToStuu]);

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!input.trim() || isLoading) return;

    sendToStuu(input.trim());
    setInput("");
  };
  ...
  return (
    <div
      ...
      className={`fixed z-50 w-[calc(100vw-3rem)] sm:w-80 max-w-80 bg-bags-dark border-4 border-green-500 shadow-lg shadow-green-500/20 chat-window-mobile ${isDragging ? "cursor-grabbing" : ""} ${isDismissing ? "modal-sheet-dismiss" : ""}`}
      {...swipeHandlers}
    >
      <div
        onPointerDown={handlePointerDown}
        className="flex items-center justify-between p-2 border-b-4 border-green-500 cursor-grab active:cursor-grabbing select-none touch-none bg-gradient-to-r from-green-600/20 to-emerald-600/20"
      >
      ...
      <div className="p-2 border-b border-green-500/30 bg-bags-darker">
        <p className="font-pixel text-[8px] text-gray-400 mb-2">Help Topics:</p>
        <div className="flex flex-wrap gap-1">
          {STUU_TOPICS.map((topic, i) => (
            <button
              key={i}
      ...
      <div className="h-40 overflow-y-auto p-2 space-y-2 chat-messages">
        {messages.length === 0 ? (
          <div className="text-center py-4">
            <p className="font-pixel text-[10px] text-green-400 mb-1">🎧 support online</p>
            <p className="font-pixel text-[8px] text-gray-400">
              Stuu here. How can I help you today?
            </p>
          ...
          messages.map((msg) => (
            ...
            >
              {msg.type === "stuu" && (
                <p className="font-pixel text-[6px] text-green-400 mb-1">Stuu:</p>
              )}
              {msg.type === "user" && (
                <p className="font-pixel text-[6px] text-bags-green mb-1">You:</p>
              )}
              {msg.type === "info" && (
                <p className="font-pixel text-[6px] text-emerald-400 mb-1">📋 Guide:</p>
              )}
              <p className="font-pixel text-[8px] text-white whitespace-pre-wrap">{msg.message}</p>
              {msg.type === "stuu" && msg.actions && msg.actions.length > 0 && (
      ...
      <div className="p-2 border-t border-green-500/30 bg-bags-darker">
        <div className="grid grid-cols-2 gap-2 text-center">
          <a
            href="https://x.com/StuuBags"
            target="_blank"
            rel="noopener noreferrer"
            className="bg-green-500/10 p-1 rounded hover:bg-green-500/20 transition-colors"
          >
            <p className="font-pixel text-[6px] text-gray-400">X/Twitter</p>
            <p className="font-pixel text-[9px] text-green-400">@StuuBags</p>
          </a>
          <div className="bg-emerald-500/10 p-1 rounded">
            <p className="font-pixel text-[6px] text-gray-400">Role</p>
            <p className="font-pixel text-[9px] text-emerald-400">Operations & Support</p>
          </div>
        </div>
