...
import { getCreatureBattleSpriteUrl, getPlayerSpriteUrl } from "@/lib/texture-bridge";
...
export function EncounterOverlay({ creature, onClose }: EncounterOverlayProps) {
  ...
  const [atkEffect, setAtkEffect] = useState<{ anim: Move["animation"]; side: "p" | "e" } | null>(
    null
  );
  ...
  const animatePlayerAttack = useCallback(
    async (move: Move, before: EncounterState, after: EncounterState) => {
      ...
      if (move.power > 0) {
        ...
        if (effMsg) {
          await typewrite(effMsg.message);
          await sleep(300);
        }
        ...
        for (const entry of logEntries) {
          if (
            entry.type === "stat_change" ||
            (entry.type === "info" && entry.message.includes("burned"))
          ) {
        ...
        // Struggle recoil
        const recoilEntry = logEntries.find(
          (e) => e.type === "info" && e.message.includes("recoil")
        );
        if (recoilEntry) {
      ...
      if (!isStatus && dmg > 0) {
        ...
        for (const entry of logEntries) {
          if (
            ...
            await typewrite(entry.message);
            if (entry.message.includes("burned")) {
              /* no extra sound */
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      sfx.faintSound();
      for (let i = 1; i <= 8; i++) {
        if (isLoss) setPlayerFaintY(i * 10);
        else setEnemyFaintY(i * 10);
        await sleep(80);
      }
      await sleep(400);
      if (s.result === "win") {
    ...
    const handle = (e: KeyboardEvent) => {
      ...
      if (vp === "player_turn") {
        if (e.key === "ArrowUp" || e.key === "w") {
          setMenuIdx((c) => Math.max(0, c - 1));
          sfx.menuBlip();
        } else if (e.key === "ArrowDown" || e.key === "s") {
          setMenuIdx((c) => Math.min(2, c + 1));
          sfx.menuBlip();
        } else if (e.key === "Enter" || e.key === "e" || e.key === " ") {
          sfx.menuSelect();
          if (menuIdx === 0) {
            if (allMovesExhausted) {
              doPlayerAttack(STRUGGLE_MOVE);
            } else {
              setMoveIdx(0);
              setVp("move_select");
            }
        ...
        if (e.key === "ArrowUp" || e.key === "w") {
          setMoveIdx((c) => Math.max(0, c - 2));
          sfx.menuBlip();
        } else if (e.key === "ArrowDown" || e.key === "s") {
          setMoveIdx((c) => Math.min(moves.length - 1, c + 2));
          sfx.menuBlip();
        } else if (e.key === "ArrowLeft" || e.key === "a") {
          setMoveIdx((c) => Math.max(0, c - 1));
          sfx.menuBlip();
        } else if (e.key === "ArrowRight" || e.key === "d") {
          setMoveIdx((c) => Math.min(moves.length - 1, c + 1));
          sfx.menuBlip();
  ...
  return (
    ...
    >
      <div className="absolute inset-0 flex items-center justify-center bg-black">
        <div
          className="relative w-full max-w-[640px] max-h-[576px] aspect-[10/9] overflow-hidden"
          style={{ imageRendering: "pixelated" }}
        >
          {/* Battle background — Pokemon Crystal style with sky, terrain, platforms */}
          <div
            className="absolute inset-x-0 top-0 bottom-[33.3%]"
            style={{
              transform: introShake
                ? `translate(${shakeOffset.x * introShake}px, ${shakeOffset.y * introShake}px)`
                : undefined,
            }}
          >
            {/* Sky gradient */}
            ...
            <div
              className="absolute left-0 right-0 top-[32%] h-[16%]"
              style={{
                background: `
            ...
            {/* Main ground plane */}
            <div
              className="absolute inset-x-0 top-[45%] bottom-0"
              style={{
                background:
                  "linear-gradient(180deg, #78b860 0%, #88c868 15%, #68a850 40%, #58a040 70%, #489838 100%)",
              }}
            />
            ...
            {/* Enemy platform */}
            <div
              className="absolute right-[2%] top-[38%] w-[45%] h-[22%]"
              style={{
            ...
            {/* Player platform */}
            <div
              className="absolute left-[-5%] bottom-0 w-[50%] h-[35%]"
              style={{
            ...
            <div className="absolute left-[2%] top-[4%] w-[48%] z-10">
              <div
                className="bg-[#f8f0d8] border-[3px] border-[#484848] p-1.5"
                style={{ boxShadow: "3px 3px 0 rgba(0,0,0,0.15)" }}
              >
                <div className="flex justify-between items-center mb-1">
                  <span className="font-pixel text-[10px] text-[#383838] uppercase">
                    {creature.name}
                  </span>
                ...
                <div className="flex items-center gap-1">
                  ...
                  <div className="flex-1 h-[6px] bg-[#282828] border border-[#181818] overflow-hidden">
                    <div
                      ...
                      style={{
                        width: hpW(enemyHp, creature.stats.maxHp),
                        backgroundColor: hpColor(enemyHp, creature.stats.maxHp),
                      }}
                    />
                  </div>
                </div>
            ...
            {/* --- ENEMY SPRITE --- */}
            <div
              className="absolute right-[8%] top-[3%] w-[30%] aspect-square z-[5]"
              style={{
                ...
                animation: enemyShake
                  ? "pkmn-shake 0.06s linear infinite"
                  : enemyBob
                    ? "pkmn-idle 0.8s ease-in-out infinite"
                    : undefined,
              }}
            >
              ...
              {creatureSpriteUrl ? (
                ...
                <div
                  ...
                  style={{ backgroundColor: typeCol, borderColor: `${typeCol}88` }}
                >
                  <span className="font-pixel text-white text-[9px] text-center drop-shadow-[1px_1px_0_rgba(0,0,0,0.5)]">
                    {creature.name}
                  </span>
                </div>
              )}
              {dmgNum && dmgNum.side === "e" && (
                <div
                  key={dmgNum.k}
                  className="absolute -top-1 left-1/2 -translate-x-1/2 font-pixel text-sm text-white pkmn-dmg-float"
            ...
            {/* --- PLAYER SPRITE --- */}
            <div
              className="absolute left-[6%] bottom-[2%] w-[26%] aspect-square z-[5]"
              style={{
                ...
                <div className="flex items-center gap-1 mb-0.5">
                  ...
                  <div className="flex-1 h-[6px] bg-[#282828] border border-[#181818] overflow-hidden">
                    <div
                      className="h-full transition-none"
                      style={{
                        width: hpW(playerHp, state.player.maxHp),
                        backgroundColor: hpColor(playerHp, state.player.maxHp),
                      }}
                    />
                  </div>
                </div>
            ...
            {/* Lunge indicators */}
            {playerLunge && (
              <div className="absolute left-[30%] bottom-[25%] w-4 h-4 bg-white/40 rounded-full pkmn-lunge-r" />
            )}
            {creatureLunge && (
              <div className="absolute right-[30%] top-[35%] w-4 h-4 bg-white/40 rounded-full pkmn-lunge-l" />
            )}
          </div>
          ...
          <div className="absolute inset-x-0 bottom-0 h-[33.3%] bg-[#f8f0e0] border-t-[4px] border-[#484848]">
            <div className="h-full flex">
                ...
                <div className="w-[55%] border-l-[4px] border-[#484848] bg-[#f8f0d8] p-1.5 flex flex-col justify-center">
                  <div className="grid grid-cols-2 gap-0.5">
                    {state.player.moves.map((m, i) => (
                      ...
                      >
                        <div className="flex items-center gap-1">
                          <span className="font-pixel text-[8px] text-[#383838] w-2.5">
                            {moveIdx === i ? "\u25B6" : ""}
                          </span>
                          <span className="font-pixel text-[9px] text-[#383838] truncate">
                            {m.name}
                          </span>
                        </div>
                        <span className="font-pixel text-[7px] text-[#888] ml-3.5">
                          PP {m.pp}/{m.maxPp}
                        </span>
                      </button>
                    ))}
              ...
              {/* Done click zone */}
              {vp === "done" && (
                <button onClick={handleDone} className="absolute inset-0 cursor-pointer" />
              )}
            </div>
          </div>
          ...
          {blackOverlay > 0 && (
            <div
              className="absolute inset-0 bg-black pointer-events-none z-20 transition-opacity duration-400"
              ...
              {[...Array(10)].map((_, i) => (
                <div
                  ...
                  style={{
                    ...
                    backgroundColor: "#000",
                    transform:
                      wipePhase === 1 ? `scaleY(0)` : wipePhase === 2 ? `scaleY(1)` : `scaleY(0)`,
                    transformOrigin: wipeDir === "close" ? "top" : "bottom",
                    animation:
                      wipePhase === 1
                        ? `wipe-bar-close 0.35s ease-out ${i * 0.025}s forwards`
                        : wipePhase === 3
                          ? `wipe-bar-open 0.4s ease-in ${i * 0.03}s forwards`
                          : undefined,
                  }}
                />
              ))}
      ...
      <style jsx global>{`
        ...
        .pkmn-lunge-r {
          animation: pkmn-lunge-r 0.2s ease-out forwards;
        }
        @keyframes pkmn-lunge-l {
          0% {
            transform: translateX(0);
            opacity: 1;
          }
        ...
        .pkmn-lunge-l {
          animation: pkmn-lunge-l 0.2s ease-out forwards;
        }
        @keyframes pkmn-slash {
          0% {
...
function AttackEffect({ anim }: { anim: Move["animation"] }) {
  ...
  switch (anim) {
      ...
      return (
        <div className="absolute inset-0 pointer-events-none overflow-hidden z-10">
          <div
            ...
            style={{ animation: "pkmn-slash 0.15s ease-out forwards" }}
          />
          <div
            className="absolute top-[20%] left-[10%] w-[80%] h-[3px] bg-white rotate-[-30deg]"
            style={{ animation: "pkmn-slash 0.2s ease-out forwards", animationDelay: "0.05s" }}
          />
        </div>
      );
    case "ember":
    ...
    case "shimmer":
      return (
        <div
          className="absolute inset-0 pointer-events-none z-10 bg-white/50"
          style={{ animation: "pkmn-shimmer 0.5s ease-in-out forwards" }}
        />
      );
    case "debuff":
      return (
        <div className="absolute inset-0 pointer-events-none z-10">
          <div
            ...
            style={{ animation: "pkmn-shimmer 0.4s ease-in-out forwards" }}
          />
        </div>
      );
    case "quick":
      return (
        <div className="absolute inset-0 pointer-events-none overflow-hidden z-10">
          ...
          <div
            className="absolute inset-0 bg-white"
            style={{ animation: "pkmn-slash 0.12s ease-out forwards" }}
          />
        </div>
      );
    default:
